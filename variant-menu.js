...
function renderPrices(item) {
  ...
  if (primary && secondary) {
    return `<div class="prices two-column"><div class="price-cell">${primaryLabel ? `<span>${primaryLabel}</span>` : ""}<strong>${primary}</strong></div><div class="price-cell">${secondaryLabel ? `<span>${secondaryLabel}</span>` : ""}<strong>${secondary}</strong></div></div>`;
  }
...
function setPage(index) {
  ...
  [...dotsEl.children].forEach((dot, dotIndex) => dot.classList.toggle("active", dotIndex === currentPage));
}
